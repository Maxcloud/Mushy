...
public class MobHandler {
    ...
    public static final void MoveMonster(LittleEndianAccessor lea, MapleClient c, MapleCharacter chr) {
        ...
        for (int i = 0; i < size1; i++) {
            unk3.add(new Pair(Integer.valueOf(lea.readShort()), Integer.valueOf(lea.readShort())));
        }
        List unk2 = new ArrayList();
        byte size = lea.readByte();
        for (int i = 0; i < size; i++) {
            unk2.add(Integer.valueOf(lea.readShort()));
        }
        lea.readByte();
        ...
        if ((res != null) && (chr != null) && (res.size() > 0)) {
            ...
            // System.out.println(lea.available() + " is available.");
            if (lea.available() != 30L) { // was 36
                System.out.println("Move_life : slea.available != 30");
                return;
            }
