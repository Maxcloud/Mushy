...
public class PlayerHandler {
	...
	public static void rangedAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
		...
		if (attack.skillid != 0) {
			...
			if ((effect.getCooldown(chr) > 0) && (!chr.isGM())
					&& (((attack.skillid != 35111004) && (attack.skillid != 35121013))
							|| (chr.getBuffSource(MapleBuffStat.MECH_CHANGE) != attack.skillid))) {
		...
		if (!chr.isHidden()) {
			if (attack.skillid == 3211006) {
				chr.getMap().broadcastMessage(chr,
						CField.strafeAttack(chr.getId(), attack.nMobCount, attack.skillid, skillLevel, attack.display,
				...
				chr.getMap().broadcastMessage(chr,
						CField.rangedAttack(chr.getId(), attack.nMobCount, attack.skillid, skillLevel, attack.display,
								attack.speed, visProjectile, attack.allDamage, attack.position, chr.getLevel(),
	...
	public static void MagicDamage(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
		...
		if (!chr.isHidden()) {
			chr.getMap().broadcastMessage(chr,
					CField.magicAttack(chr.getId(), attack.nMobCount, attack.skillid, skillLevel, attack.display,
							attack.speed, attack.allDamage, attack.charge, chr.getLevel(), attack.unk),
					chr.getTruePosition());
		} else {
			chr.getMap().broadcastGMMessage(chr, CField.magicAttack(chr.getId(), attack.nMobCount, attack.skillid, skillLevel, attack.display, attack.speed, attack.allDamage, attack.charge, chr.getLevel(), attack.unk), false);
		}
