...
public class MaplePyramidSubway {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    ...
                    chr.getClient().getSession().write(CField.showMapEffect("killing/first/number/" + stage));
                    chr.getClient().getSession().write(CField.showMapEffect("killing/first/stage"));
                    chr.getClient().getSession().write(CField.showMapEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().getSession().write(CField.getClock(time));
            c.getClient().getSession().write(CField.showMapEffect("killing/first/number/" + stage));
            c.getClient().getSession().write(CField.showMapEffect("killing/first/stage"));
            c.getClient().getSession().write(CField.showMapEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.getClient().getSession().write(CField.showMapEffect("killing/clear"));
    ...
    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().getSession().write(CField.showMapEffect(effect));
    }
    ...
    private static void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().getSession().write(CField.showMapEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().getSession().write(CField.showMapEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().getSession().write(CField.showMapEffect("killing/fail"));
        }
