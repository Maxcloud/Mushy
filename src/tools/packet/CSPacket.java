...
public class CSPacket {
    ...
    public static byte[] CS_Picture_Item() {
        ...
        pw.write(4);
        CashItemFactory cif = CashItemFactory.getInstance();
        pw.write(cif.getMenuItems(409).size() > 0 ? 1 : 3);
        pw.write(cif.getMenuItems(409).size());
...
//        }
        ...
        if (pack == null) {
            ...
            for (int i = 0; i < pack.size(); i++) {
                pw.writeInt(100000677);//item.getSN()); //should be pack item sn
                pw.writeInt(1072443);//((Integer) pack.get(i)).intValue());
                pw.writeInt(1);//1
                pw.writeInt(3600); //pack item usual price
                pw.writeInt(2880); //pack item discounted price
                pw.writeInt(0);
                pw.writeInt(1);
                pw.writeInt(0);
                pw.writeInt(2);
